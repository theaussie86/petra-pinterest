...
const ACCEPTED_TYPES = 'image/*,video/mp4,video/quicktime,video/webm'
...
/** Check if aspect ratio is approximately 2:3 (ratio 0.6-0.7) */
...
export function MediaUploadZone({ files, onFilesChange, disabled }: MediaUploadZoneProps) {
        ...
        if (isVideoFile(file)) {
          ...
          video.onseeked = () => {
            setThumbnails((prev) => {
              ...
              updated.set(key, { url, width: video.videoWidth, height: video.videoHeight, isVideo: true })
              return updated
            })
          }
          video.src = url
          // Set placeholder until loaded
          newThumbnails.set(key, { url, width: 0, height: 0, isVideo: true })
        } else {
          ...
          img.onload = () => {
            setThumbnails((prev) => {
              const updated = new Map(prev)
              updated.set(key, { url, width: img.naturalWidth, height: img.naturalHeight, isVideo: false })
              return updated
            })
          }
          img.src = url
          // Set placeholder until loaded
          newThumbnails.set(key, { url, width: 0, height: 0, isVideo: false })
        }
      }
    })
          ...
          {files.map((file, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Video badge */}
                {info?.isVideo && (
                  <span className="absolute bottom-[calc(100%-2rem)] left-1 rounded bg-black/70 px-1.5 py-0.5 text-[10px] font-medium text-white">
