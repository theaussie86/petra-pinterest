...
export const refreshPinterestTokens = inngest.createFunction(
  { id: 'refresh-pinterest-tokens', retries: 3 },
  ...
  async ({ step }) => {
    ...
    if (!connections || connections.length === 0) {
      return { refreshed: 0, failed: 0, message: 'No tokens need refreshing' }
    }

    let totalRefreshed = 0
    let totalFailed = 0
    const details: Array<{ connection_id: string; success: boolean; error?: string }> = []

    // Process each connection
    for (const connection of connections) {
      const result = await step.run(`refresh-token-${connection.id}`, async () => {
        try {
          // Get refresh token from Vault
          const { data: refreshTokenData, error: vaultError } = await supabase.rpc(
            'get_pinterest_refresh_token',
            { p_connection_id: connection.id }
          )

          if (vaultError || !refreshTokenData) {
            throw new Error(`Failed to retrieve refresh token: ${vaultError?.message || 'Unknown error'}`)
          }
          ...
          // Calculate new expiration time (expires_in is in seconds)
          const expiresAt = new Date()
          expiresAt.setSeconds(expiresAt.getSeconds() + tokenResponse.expires_in)

          // Store new tokens in Vault
          const { error: storeError } = await supabase.rpc('store_pinterest_tokens', {
            p_connection_id: connection.id,
          ...
          if (storeError) {
            throw new Error(`Failed to store new tokens: ${storeError.message}`)
          }
      ...
      if (result.success) {
        ...
        totalFailed++
        details.push({ connection_id: connection.id, success: false, error: result.error })
      }
    }
