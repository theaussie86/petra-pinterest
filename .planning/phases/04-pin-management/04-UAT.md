...
number: 1
name: Create Pin with Image Upload
expected: |
  On a project detail page, click "Create Pin". A dialog opens with an article selector, optional board selector, and an image upload zone. Select an article, drag-drop or pick an image file. The image preview appears with a thumbnail, file name, and file size. Click submit — the pin is created and appears in the project's pins list.
