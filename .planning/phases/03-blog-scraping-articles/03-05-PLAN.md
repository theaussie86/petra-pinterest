  ...
  <action>
    ...
    **`scrapeBlogFn`** -- Full blog scrape (POST method):
    ...
    - Uses `createServerFn({ method: 'POST' })` with `.inputValidator()` for input validation (use a plain validator function, not Zod -- keeps it simple):
      ...
      ```
    - Creates a Supabase client server-side using `process.env.VITE_SUPABASE_URL` and `process.env.VITE_SUPABASE_PUBLISHABLE_KEY` with the user's `access_token` in the `Authorization: Bearer` header (same pattern as the Edge Function)
