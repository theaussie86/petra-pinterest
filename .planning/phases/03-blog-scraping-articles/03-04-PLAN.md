...
must_haves:
  ...
  key_links:
    - from: "src/routes/_authed/projects/$projectId/articles/$articleId.tsx"
      to: "src/lib/hooks/use-articles.ts"
      via: "useArticle hook"
      pattern: "useArticle"
    - from: "src/components/articles/articles-table.tsx"
      to: "src/routes/_authed/projects/$projectId/articles/$articleId.tsx"
      via: "TanStack Router Link"
      pattern: "Link.*articles"
---
...
<task type="auto">
  ...
  <files>src/routes/_authed/projects/$projectId/articles/$articleId.tsx</files>
  <action>
...
**Content sanitization — create a simple sanitizeHtml helper:**
...
Import Header from `@/components/layout/header`, Button from `@/components/ui/button`, Card/CardContent from `@/components/ui/card`, icons from lucide-react (ArrowLeft, ExternalLink), hooks from `@/lib/hooks/use-articles`.
  ...
  <verify>
File exists at the correct route path. Run `npm run dev` briefly to trigger route tree generation, then run `npm run build` to verify TypeScript compilation. Check that `src/routeTree.gen.ts` includes the new article detail route.
  </verify>
  <done>
Article detail page renders with metadata header (title, dates, source URL, pin count), action buttons (archive), and full article content with HTML sanitization. Back navigation returns to the project. Route tree is auto-generated.
  </done>
</task>

<task type="auto">
  <name>Task 2: Wire article detail navigation from articles table</name>
  <files>src/components/articles/articles-table.tsx</files>
  <action>
Update the ArticlesTable component (created in Plan 03) to ensure the title column links correctly to the article detail page.

The title column should use TanStack Router `<Link>`:
```tsx
<Link
...
**Note:** This task should only be executed AFTER Plan 03 has completed (since Plan 03 creates the articles-table.tsx file). If Plan 03 and Plan 04 run in parallel, this wiring may need to be done as a post-step. The executor should check if articles-table.tsx exists first. If it doesn't exist yet (Plan 03 hasn't run), skip this task with a note — the link will be wired when both plans are complete.

However, since both Plan 03 and 04 are in Wave 3, and Plan 03 creates articles-table.tsx while Plan 04 modifies it, there IS a file conflict. To resolve: Plan 03 should already include the Link to the article detail route (the plan action already specifies "clickable links to article detail page"). So this task just needs to VERIFY the link is correct, not create it.

**Verification-only task:** Confirm that articles-table.tsx (from Plan 03) has a Link component pointing to `/articles/$articleId` with correct params. If the link path doesn't match the route created in Task 1, update it.
  </action>
  <verify>
`npm run build` passes. Click a title in the articles table navigates to the article detail page (verify by checking the Link `to` prop matches the route path).
  </verify>
  <done>
Article titles in the table link to the article detail page. Navigation between articles table and article detail works correctly in both directions.
  </done>
</task>
