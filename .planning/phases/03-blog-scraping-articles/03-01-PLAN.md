...
must_haves:
  truths:
    ...
    - "Scrape Edge Function supports single_url parameter for manual article addition"
  artifacts:
...
```json
{
  ...
  "blog_url": "https://example.com",     // required for full scrape mode
  "rss_url": "https://example.com/feed", // optional, used in full scrape mode
  "single_url": "https://example.com/post-slug" // optional, if provided: scrape only this URL (manual add mode)
}
```
...
**Mode A — Single URL (manual add):** If `single_url` is provided, skip blog index/RSS scraping entirely. Instead:
- Fetch the single URL directly
- Extract title from `<h1>` or `<title>`, content from `<article>` or main content area, date from `<time>` or meta tags
- Upsert the single article into blog_articles
- Return ScrapeResponse with articles_found = 0 or 1

**Mode B — Full scrape (blog index):** If `single_url` is NOT provided, perform the full scraping logic below.

**Scraping logic (Mode B — full scrape):**
...
File exists at `supabase/functions/scrape-blog/index.ts`. Verify syntax by reviewing the file. Deploy with `supabase functions deploy scrape-blog` (may require Supabase CLI auth). If deploy is not possible now, verify the file has correct Deno syntax and exports.
  ...
  <done>
Edge Function file exists with two operating modes: single_url mode for manual article addition, and full scrape mode with RSS parsing + HTML fallback. Both modes upsert articles. CORS headers and proper error handling included. Function is deployable via Supabase CLI.
  </done>
</task>
...
5. Edge Function handles both RSS and HTML scraping paths (Mode B)
6. Edge Function handles single_url parameter for manual article addition (Mode A)
7. `npm run build` passes (no TypeScript regressions)
</verification>
