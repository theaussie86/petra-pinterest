...
<task type="auto">
  ...
  <action>
    ...
    **3. Update `.env.example`** — Add new env vars (append, don't overwrite existing):
    ...
    ```
    Note: The server uses `SUPABASE_URL` (not `VITE_SUPABASE_URL`) and `SUPABASE_SERVICE_ROLE_KEY` (not anon key) because the server writes articles on behalf of users. The service role bypasses RLS, so the server must enforce tenant isolation in code.
  ...
  <verify>
    ...
    - `.env.example` contains INNGEST_EVENT_KEY, SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY
...
<task type="auto">
  ...
  <action>
    **1. Create `server/inngest/functions/scrape-blog.ts`:**
    ...
    - Use `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` env vars (not user JWT — the server is trusted)
    ...
    Then implement the helper functions (`scrapeRss`, `discoverRssFeed`, `scrapeHtml`, `scrapeSingleUrl`) using `node-html-parser` and `fast-xml-parser` instead of Deno's `DOMParser`. Port the exact same logic from the Edge Function but adapt the parsing API:

    For HTML parsing with `node-html-parser`:
    - `parse(html)` returns a root node
    - `root.querySelector('h1')?.textContent` works the same
    - `root.querySelector('article')?.innerHTML` works the same
    - `root.querySelectorAll('item')` works the same

    For XML/RSS parsing with `fast-xml-parser`:
    - Use `new XMLParser({ ignoreAttributes: false, attributeNamePrefix: '@_' })`
    - Access RSS items via `parsed.rss?.channel?.item` (array or single object)
    - Access Atom entries via `parsed.feed?.entry`
    - Links in Atom: `entry.link?.['@_href']` or if array, find the one with `@_rel === 'alternate'`

    Key differences from Edge Function:
    - `fetch()` is available natively in Node.js 18+ (no polyfill needed)
    - Use `node-html-parser`'s `parse()` instead of `new DOMParser().parseFromString()`
    - Use `fast-xml-parser`'s `XMLParser` instead of `DOMParser` for XML
